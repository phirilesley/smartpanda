...
public class FeeStructuresController(SmartSchoolDbContext dbContext) : ControllerBase
{
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<FeeStructure>> GetById(Guid id, CancellationToken cancellationToken)
    {
        var item = await dbContext.FeeStructures.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
